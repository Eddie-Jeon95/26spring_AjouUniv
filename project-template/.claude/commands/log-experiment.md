...
- `metrics.json`
- `test_metrics.json`
- classification이면 `confusion_matrix.json`
- binary classification이면 `threshold_metrics.csv`
- AutoGluon 실험이면 `leaderboard.csv`와 `automl_summary.json`
...
- AutoGluon이면 leaderboard_path와 automl_summary_path가 기록됐는가?
- task_type, positive_class, auxiliary_metrics가 기록됐는가?
...
- AutoGluon 결과는 baseline과 같은 data_version, split, metric 조건에서만 직접 비교하세요.
