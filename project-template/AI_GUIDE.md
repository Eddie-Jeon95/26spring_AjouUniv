...
처음에는 아래 순서만 따라가도 됩니다.
각 단계에서 Claude가 만든 결과를 그대로 믿기보다, 오른쪽의 확인 질문을 사람이 함께 봅니다.

| 단계 | Claude에게 말할 것 | 사람이 확인할 것 |
|------|-------------------|------------------|
| 1. 문제 정의 | `PROJECT_SPEC.md를 읽고 이 데이터로 풀 문제, target, 주요 metric 초안을 잡아줘.` | 어떤 오류가 더 위험한가? metric이 문제와 맞는가? |
| 2. raw 데이터 설명 | `raw 파일은 data/raw/[파일명]이고 target은 [target]이야. header/컬럼 정보는 ...야.` | 파일 경로, target, 컬럼 순서가 맞는가? |
| 3. EDA 실행 | `/eda data/raw/[파일명] target=[target]` | target 분포, 결측, 중복, 이상치, 누수 의심을 확인했는가? |
| 4. Data Card 정리 | `EDA 결과를 DATA_CARD.md에 반영하고, 모델링 전 전처리 결정을 정리해줘.` | Claude의 해석이 그래프/표와 맞는가? |
| 5. 전처리 | `/preprocess-data data/raw/[파일명] target=[target] output=data/processed/[버전].csv` | fit이 필요한 scaler/encoder/imputer를 미리 적용하지 않았는가? |
| 6. Baseline 학습 | `/train-baseline data/processed/[버전].csv target=[target] data_version=[버전] test_size=0.2 val_size=0.2` | split, seed, metric, confusion matrix가 기록됐는가? |
| 7. 결과 해석 | `방금 baseline 결과를 문제 정의 기준으로 해석해줘. accuracy, macro F1, FP/FN 의미를 설명해줘.` | metric 숫자만 보고 결론 내리지 않았는가? |
| 8. 다음 실험 계획 | `/plan-experiment [실험 아이디어]` 또는 `다음 실험 후보를 3개 제안해줘.` | 실험이 하나의 가설과 비교 기준을 갖는가? |
| 9. 모델 비교 | `/compare-models` 또는 `최근 두 모델을 같은 data_version/split 기준으로 비교해줘.` | data_version, split, seed가 달라 직접 비교하면 안 되는지 확인했는가? |
| 10. 데모와 checkpoint | `/check-streamlit` 후 `/checkpoint [단계 요약]` | 앱에서 모델 선택/예측/로그가 보이는가? commit에 데이터/모델 파일이 들어가지 않았는가? |

### Slash command와 그냥 말의 차이

Slash command는 반복되는 작업 절차를 고정하기 위한 장치입니다.
EDA, 전처리, baseline 학습, Streamlit 점검, checkpoint처럼 산출물이 정해진 작업에 사용하세요.

그냥 말로 질문해도 됩니다.
해석, 판단, 다음 실험 아이디어, metric 의미, 오류 원인처럼 생각이 필요한 대화는 자연어로 묻는 편이 좋습니다.
...
| 커맨드 | 용도 | 언제 사용 |
|--------|------|-----------|
| `/eda data/raw/... target=...` | EDA 계획, notebook 실행, Data Card 초안 | raw 데이터를 처음 확인할 때 |
| `/preprocess-data data/raw/... target=... output=...` | raw 데이터를 학습용 processed CSV로 정리 | EDA 후 baseline 전 |
| `/train-baseline data/processed/... target=... data_version=...` | baseline 학습 전 계획과 실행 기록 | processed 데이터가 준비된 뒤 |
| `/check-streamlit` | Streamlit 앱 실행 전 산출물 연결 점검 | baseline 학습 후 데모 실행 전 |
| `/checkpoint [메시지]` | 문서/코드 변경을 검토하고 checkpoint commit | EDA/전처리/학습/Streamlit 단계가 끝난 뒤 |
...
새로 추가한 slash command가 기존 Claude Code 세션에서 보이지 않으면, 프로젝트 루트에서 새 세션을 열거나 해당 command 파일의 내용을 붙여 넣어 같은 요청을 하세요.
패키지가 없을 때는 특정 패키지를 최신 버전으로 따로 설치하기보다 `pip install -r requirements.txt`를 우선 사용합니다.
